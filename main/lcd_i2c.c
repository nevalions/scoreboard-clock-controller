...
static esp_err_t lcd_i2c_write_nibble(LcdI2C *lcd, uint8_t nibble,
                                      bool is_data);
...
bool lcd_i2c_begin(LcdI2C *lcd, uint8_t addr, gpio_num_t sda_pin,
                   gpio_num_t scl_pin) {
  if (!lcd) {
  ...
  i2c_master_bus_config_t i2c_mst_config = {
      ...
      .sda_io_num = sda_pin,
      .glitch_ignore_cnt = 7,
      .flags.enable_internal_pullup = (sda_pin < 34 && scl_pin < 34),
  };
  ...
  if (ret != ESP_OK) {
    ...
    ESP_LOGE(TAG, "Check SDA pin %d and SCL pin %d connections", sda_pin,
             scl_pin);
    return false;
  }
...
static esp_err_t lcd_i2c_write_nibble(LcdI2C *lcd, uint8_t nibble,
  ...
  ESP_LOGD(TAG, "Writing nibble: 0x%01X -> 0x%02X (data=%d)", nibble, data,
           is_data);
...
static esp_err_t lcd_i2c_pulse_enable(LcdI2C *lcd, uint8_t data) {
  ...
  ESP_LOGD(TAG, "Enable pulse: high=0x%02X, low=0x%02X", enable_high,
           enable_low);

  esp_err_t ret =
      i2c_master_transmit(lcd->i2c_dev, &enable_high, 1, 100); // 100ms timeout
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "I2C enable high failed: %s, data: 0x%02X",
             esp_err_to_name(ret), enable_high);
    return ret;
  }
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "I2C enable low failed: %s, data: 0x%02X",
             esp_err_to_name(ret), enable_low);
    return ret;
  }
  vTaskDelay(pdMS_TO_TICKS(1));
...
static void lcd_i2c_write_8bit(LcdI2C *lcd, uint8_t data, bool is_data) {
  ...
  ESP_LOGD(TAG, "8-bit write: data=0x%02X, is_data=%d, send_data=0x%02X", data,
           is_data, send_data);
...
void lcd_debug_pin_scan(LcdI2C *lcd) {
  ...
  ESP_LOGI(TAG,
           "Each bit will be set HIGH for 1 second, then LOW for 1 second");
